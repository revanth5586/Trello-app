...
const AddNew = ({ type, parentId }) => {
 ...
 console.log("type", type);
  ...
  return (
    <div>
      <button onClick={openForm}>+ Add {type ? "a card" : " item list"}</button>
      {isFormVisible && (
        <form onSubmit={submitHandler} className="mt-3">
          ...
          <span></span>
          <div className="mt-3">
            <button type="button"    onClick={hideForm} className="mr-3  py-1 ">
              Cancel
            </button>
            <button type="submit" className="px-3 py-1 bg-purple-900  text-white">
              Save
            </button>
          </div>
        </form>
